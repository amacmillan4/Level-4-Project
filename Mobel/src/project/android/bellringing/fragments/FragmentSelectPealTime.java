...
public class FragmentSelectPealTime extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup parent, Bundle savedInstanceState){
		...
		if (SingletonData.get(getActivity()).getSetup().getPealTime().substring(0,1).equals("1")){
			nmMins.setMaxValue(15);
			nmMins.setMinValue(1);
			nmMins.setDisplayedValues(Arrays.copyOfRange(mins, 15, 30));
			nmMins.setValue((Integer.parseInt(SingletonData.get(getActivity()).getSetup().getPealTime().substring(2,SingletonData.get(getActivity()).getSetup().getPealTime().length())))/2 - 14);
			...
			nmMins.setValue(Integer.parseInt(SingletonData.get(getActivity()).getSetup().getPealTime().substring(2,SingletonData.get(getActivity()).getSetup().getPealTime().length()))/2 + 1);
		...
		nmHours.setValue(Integer.parseInt(SingletonData.get(getActivity()).getSetup().getPealTime().substring(0,1)));
		...
		btnPeal.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				SingletonData.get(getActivity()).getSetup().setPealTime(nmHours.getValue() + ":" + minutes);
				getActivity().finish();
